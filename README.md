...
XenWTN turns multiple *Wooting 60HE V2* (*analog* hall effect keyboards popular with gamers: https://wooting.io/wooting-60he-v2) into a configurable "Lumatone" (https://www.lumatone.io/) style velocity-sensitive microtonal music controller with aftertouch, 4mm key travel and per-key configurable RGB lighting.
...
The project is intended to be installed and run headless on a Raspberry Pi 4/5 with a PiSound soundcard and a recent Patchbox OS (https://community.blokas.io/t/beta-patchbox-os-bookworm-arm64-2024-04-04/5163), but will likely run on any Linux and hardware. 
...
- Two-board support (four-board support planned): use one or two keyboards as `Board0` / `Board1` with independent layouts.
